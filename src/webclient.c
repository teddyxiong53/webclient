...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (count > size)
    {
        myloge("read line failed. The line data length is out of buffer size(%d)!", count);
                                     ...
                                     const char *url, const char **request)
{
    int rc = WEBCLIENT_OK;
    char *ptr;
    char postr[6] = "80"; /* default port of 80(http) */
    const char *poptr;
    const char *path_ptr;
    ...
    *request = path_ptr ? path_ptr : "/";
    ...
    /* resolve port */
    poptr = strstr(host_addr + host_addr_len, ":");
    if (poptr && path_ptr && (poptr < path_ptr))
    {
        int polen = path_ptr - poptr - 1;

        strncpy(postr, poptr + 1, polen);
        postr[polen] = '\0';
    }

    if (poptr && (!path_ptr))
    {
        strcpy(postr, poptr + 1);
    }
    ...
    mylogd("host address: %s , port: %s", session->host, postr);

#ifdef WEBCLIENT_USING_MBED_TLS
    if (session->tls_session)
    {
        session->tls_session->port = web_strdup(postr);
    ...
    /* resolve the host name. */
    {
        ...
        ret = getaddrinfo(session->host, postr, &hint, res);
        if (ret != 0)
        {
            myloge("getaddrinfo err: %d '%s'.", ret, session->host);
            rc = -WEBCLIENT_ERROR;
            goto __exit;
        }
    }
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    {
        myloge("no memory for tls_session buffer!");
        return -WEBCLIENT_ERROR;
    }

    if((tls_ret = mbedtls_client_init(session->tls_session, (void *)pers, strlen(pers))) < 0)
    {
        myloge("initialize https client failed return: -0x%x.", -tls_ret);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (strncmp(URI, "https://", 8) == 0)
    {
        ...
        {
            myloge("connect failed, https client open URI(%s) failed!", URI);
            return -WEBCLIENT_ERROR;
        }
        session->is_tls = RT_TRUE;
#else
        myloge("not support https connect, please enable webclient https configure!");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
#endif
    }
    ...
    if (rc != WEBCLIENT_OK)
    {
        myloge("connect failed, resolve address error(%d).", rc);
        goto __exit;
    }
    ...
    {
        myloge("connect failed, resolve request address error.");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
    }
    ...
    if (session->tls_session)
    {
        ...
        {
            myloge("connect failed, https client context return: -0x%x", -tls_ret);
            return -WEBCLIENT_ERROR;
        }

        if ((tls_ret = mbedtls_client_connect(session->tls_session)) < 0)
        {
            myloge("connect failed, https client connect return: -0x%x", -tls_ret);
...

    {
        ...
        if (socket_handle < 0)
        {
            myloge("connect failed, create socket(%d) error.", socket_handle);
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0)
        {
            ...
            myloge("connect failed, connect socket(%d) error.", socket_handle);
            close(socket_handle);
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    if (length < 0)
    {
        myloge("add fields header data failed, return length(%d) error.", length);
        return -WEBCLIENT_ERROR;
    }
    va_end(args);
    ...
    {
        myloge("not enough header buffer size(%d)!", session->header->size);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    else
    {
        if (method != WEBCLIENT_USER_METHOD)
        {
            ...
            if (memcmp(header, "HTTP/1.", strlen("HTTP/1.")))
            {
                ...
                {
                    myloge("no memory for header buffer!");
                    rc = -WEBCLIENT_NOMEM;
                    goto __exit;
                }
            ...
            if (session->header->length > session->header->size)
            {
                myloge("send header failed, not enough header buffer size(%d)!", session->header->size);
    ...
    /* get and echo request header data */
    {
        ...
        for(header_str = session->header->buffer; (header_ptr = strstr(header_str, "\r\n")) != RT_NULL; )
        {
            ...
            {
                mylogd("%.*s", header_line_len, header_str);
            }
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    if (mime_ptr == RT_NULL)
    {
        myloge("no memory for get http status code buffer!");
        return -WEBCLIENT_NOMEM;
    }
...
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    if (session == RT_NULL)
    {
        myloge("webclient create failed, no memory for webclient session!");
        return RT_NULL;
    }
    ...
    {
        myloge("webclient create failed, no memory for session header!");
        web_free(session);
        session = RT_NULL;
        return RT_NULL;
    }
    ...
    {
        myloge("webclient create failed, no memory for session header buffer!");
...
int webclient_get(struct webclient_session *session, const char *URI)
{
    ...
    mylogd("get position handle response(%d).", resp_status);
...
int webclient_post(struct webclient_session *session, const char *URI, const char *post_data)
{
    ...
    if (post_data)
    {
        ...
        resp_status = webclient_handle_response(session);
        mylogd("post handle response(%d).", resp_status);
    }
...
#endif
            mylogd("receive data error(%d).", bytes_read);
    ...
    *response = RT_NULL;
    ...
    if (session->content_length < 0)
    {
        ...
        while (1)
        {
            ...
            if (new_resp == RT_NULL)
            {
                myloge("no memory for realloc new response buffer!");
                break;
            }
...
int webclient_request_header_add(char **request_header, const char *fmt, ...)
{
    ...
    if (*request_header == RT_NULL)
    {
        ...
        if (header == RT_NULL)
        {
            myloge("No memory for webclient request header add.");
            return RT_NULL;
        }
    ...
    if (length < 0)
    {
        myloge("add request header data failed, return length(%d) error.", length);
        return -WEBCLIENT_ERROR;
    }
    va_end(args);
    ...
    {
        myloge("not enough request header data size(%d)!", WEBCLIENT_HEADER_BUFSZ);
        return -WEBCLIENT_ERROR;
    }
...
int webclient_request(const char *URI, const char *header, const char *post_data, unsigned char **response)
{
    ...
    {
        myloge("request get failed, get response data cannot be empty.");
        return -WEBCLIENT_ERROR;
    }
